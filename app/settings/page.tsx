...
export default function SettingsPage() {
  ...
  async function load() {
    const [settingsRes, binRes, cookieYtdlpRes, cookieGdlRes] = await Promise.all([
      fetch('/api/settings'),
      fetch('/api/binaries'),
      fetch('/api/cookies/ytdlp'),
      fetch('/api/cookies/gallerydl'),
    ]);
    setSettings(await settingsRes.json());
    setBinaries(await binRes.json());
    setCookies({ ytdlp: await cookieYtdlpRes.json(), gallerydl: await cookieGdlRes.json() });
  }
